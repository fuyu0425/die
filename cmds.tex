...
\newenvironment{itemizesq}[1][]{
  \begin{itemize}[noitemsep,nolistsep,#1]
      \setlength{\itemsep}{-0pt}
    }{
  \end{itemize}
}

\newenvironment{enumeratesq}[1][]{
  \begin{enumerate}[noitemsep,nolistsep,#1]
      \setlength{\itemsep}{-0pt}
    }{
  \end{enumerate}
}
...
\newenvironment{itemizesqn}[1][]{
  \begin{itemize}[noitemsep,nolistsep,leftmargin=*,#1]
      \setlength{\itemsep}{-0pt}
    }{
  \end{itemize}
}

\newenvironment{enumeratesqn}[1][]{
  \begin{enumerate}[noitemsep,nolistsep,leftmargin=*,#1]
      \setlength{\itemsep}{-0pt}
    }{
  \end{enumerate}
}
...
\newcommand{\listingautoref}[1]{%
  {\renewcommand{\figureautorefname}{Listing}\autoref{#1}}%
}

\usepackage{refcount} % provides \getpagerefnumber

% autoref with page number if not in the same page!
\NewDocumentCommand{\autorefp}{s m}{%
  \IfBooleanTF{#1}{%
    % starred version - no hyperlink
    \autoref*{#2}%
  }{%
    % unstarred version - with hyperlink
    \autoref{#2}%
  }%
...
% --- Backup original \autoref ---
\let\oldautoref\autoref

% --- Redefine \autoref to include page number only if not on current page ---
\if 0
...
% Define a custom command with star form
\NewDocumentCommand{\page}{s m}{%
  \IfBooleanTF{#1}{%
    % starred version - no hyperlink
    page~\pageref*{#2}%
  }{%
