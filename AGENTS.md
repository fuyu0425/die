...
- Update `tables/`; do not edit files under `data/` directly.
- Treat `data/` as immutable. Only update it via a sync script or deliberate manual copy.

---

## CRITICAL: DO NOT MODIFY p.bib

NEVER, EVER modify `p.bib` without EXPLICIT user permission.

- DO NOT add new BibTeX entries
- DO NOT edit existing entries
- DO NOT "helpfully" add citations you think are needed
- If you need to add a citation, ASK THE USER FIRST

This is NON-NEGOTIABLE. Violating this rule causes serious problems.
...
├── pyplot/         # Pyplot scripts (py→SVG→PDF)
├── tables/         # Python table generators (py→.tex)
├── plots/          # Python plot generators (py→.pdf)
├── sty/            # Style files
...
- `\PN{}` → paragraph header without auto-period (use when header contains LaTeX commands)
...
Important: If your paragraph header contains LaTeX commands, use `\PN{}` instead of `\PP{}` to avoid build issues.
...
- Tables: `tables/*.py` → `tables/*.tex` via python (ARGS convention)
- Plots: `plots/*.py` → `plots/*.pdf` via python (ARGS convention)
- Pyplot: `pyplot/*.py` → `pyplot/*.svg` → `pyplot/*.pdf`
...
- DO NOT modify `p.bib` without explicit user permission
...
- NEVER modify `p.bib` without asking the user first
